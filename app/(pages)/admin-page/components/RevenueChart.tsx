...
export default function RevenueChart() {
  ...
  useSWR('getRevenuePerWeekReview', async () => {
    ...
    [
      {
        title: "12 Tháng",
        labels: yearlabels,
        apiUrl: "/api/request/request-per-month"
      },
      {
        title: "30 Ngày",
        labels: monthLabels,
        apiUrl: "/api/request/request-per-week"

      },
      {
        title: "7 Ngày",
        labels: weekLabels,
        apiUrl: "/api/request/request-per-day"
      },
    ]
    ...
    datasets: [
      {
        ...
        borderWidth: 2,
        fill: false,
        data: labels.map((e, i) => fetchResult?.data && fetchResult?.data[i]).reverse(),
      },
    ],
  };
  ...
  const handlePreviewXlsx = () => {
    ...
    const datas: any[] = []
    const yearObject = generateObject(yearlabels.map(yearlabel => yearlabel.toLowerCase()), yearData);
    const monthObject = generateObject(monthLabels.map(yearlabel => yearlabel.toLowerCase()), monthData);
    const weekObject = generateObject(weekLabels.map(yearlabel => yearlabel.toLowerCase()), weekData);
  ...
  return (
    <div className='border rounded-lg p-6'>
      <div className='flex justify-between'>
        ...
        <div>
          {chartOption.map((e, i) => (
            <span onClick={() => setCurrentOptionIndex(i)} className={cn('py-1 text-sm px-2 mx-1 hover:border rounded-lg cursor-pointer', currentOptionIndex == i && 'border')} key={i}>{e.title}</span>
          ))}
        </div>
        <div onClick={handlePreviewXlsx}>
          <PreviewXlsx columns={xlsxColumns} data={xlsxData} />
        </div>
      </div>
      <Chart
        height={115}
