...
function ServiceItemPage({ params }: { params: { serviceId: string } }) {
    ...
    <div className="w-full flex flex-col items-center mt-12 ">
      ...
      <div className="shadow-lg flex flex-col justify-center w-11/12 rounded-lg p-8 mt-3  border-t">
        <div className="flex items-center justify-between">
          <div>
            Danh sách item của dịch vụ{" "}
            <span className="text-crusta"> {data?.serviceName} </span>
          </div>
          <Link
            href={`/manager-page/service-management/${params.serviceId}/service-item/add`}
            className=""
          >
      ...
      <div className="w-11/12">
        <ServiceItemTable serviceId={params.serviceId} />
      </div>
    </div>
  );
}
